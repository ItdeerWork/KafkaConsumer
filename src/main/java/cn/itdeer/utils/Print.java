...
public class Print {
    ...
    public static void outPrint(long startTime, long endTime, String startDate, String endDate, String threadName) {
        ...
        System.out.printf("\n线程数 | " + "分区数 | " + "副本数 | " + "开始时间 | " + "结束时间 | " + "共用（秒） | " + "消息大小（字符） | " + "总数据大小（MB） | " + "总消息数量（条） | " + "吞吐量（MB/S） | " + "速度（条/秒） | " + "\n" + "---|---|---|---|---|---|---|---|---|---|---" + "\n" + "%s | " + "%s | " + "%s | " + "%s | " + "%s | " + "%s | " + "%s | " + "%s | " + "%s | " + "%s | " + "%s \n", Constants.THREAD_NUMS, Constants.PARTITION_NUMS, Constants.REPLICATION_NUMS, startDate, endDate, totle_time, Constants.MESSAGE_SIZE_NUMS, totle_size, totle_nums, speed_size, speed);
        System.out.println();
    }
}
